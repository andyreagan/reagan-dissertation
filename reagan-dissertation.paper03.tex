...
To the extend possible, all figures and analysis can be reproduced by running a single script.
Below, we include a reprint of Figure 1, Figure S1, and the power law fit tables from the paper.
...
\begin{figure}[tbp!]
  ...
  \caption[]{
    Teletherm dials shows the yearly temperature dynamics for a single location over a period of time, and time series below show the trends for both temperature extremes and teletherm dates.
    The min and max temperature for each day of the year are smoothed over three 25 year windows, one for each dial, and show in blue and red, respectively.
...
After collect the Tweets for these hashtags, they were reorganized by user, and then collected into a \verb|sqlite| database using Django, a Python web framework.
