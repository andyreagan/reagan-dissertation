...
Used n-grams from 4 billion web pages, and pruned a network with various heuristics constructed from context vectors (window size 6).
Seeds words for positive and negative sentiment, and scores determined by shortest paths to the seed set, a ``graph propagation algorithm''.
...
Utilizes the approach set out in \cite{velikovich2010viability} to generate corpus specific word embeddings using SVD and propagate sentiment labels on inferred $k$-NN network.
